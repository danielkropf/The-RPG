...
public class addAP : MonoBehaviour {
	...
	public void addMaxAP(string atributo) {
		...
		switch (atributo) {
			...
			case "LUK":
				...
				diferenca = tw.tst.maxAP - tw.tst.luk;
				if (tw.tst.remainingAP >= diferenca)
				{
					tw.tst.luk += diferenca;
					tw.tst.remainingAP -= diferenca;
				}
				else
				{
	...
	public void removeMaxAP(string atributo) {
		...
		case "LUK":
			...
			diferenca = tw.tst.luk - tw.tst.minAP;
			tw.tst.luk -= diferenca;
			tw.tst.remainingAP += diferenca;
	...
	void Update() {
		...
		case "pointsLUK":
			txt = this.GetComponent<Text>();
			txt.text = tw.tst.lukTotal.ToString();

			txt = lukPlus.GetComponent<Text>();
			txt.text = "+ (" + tw.tst.luk.ToString() + ")";
			break;
